This thing doesn't work.  Any SetEnvironmentVar calls that pass "User" silently hang.  
Commented them all out, but git continues to prompt me for auth.
...
function Start-SshAgent() {
    ...
    foreach ($line in $lines) {
        if (([string]$line).Trim() -match "(.+)=(.*)") {
			Write-Host "Setting output of $line"
			...
			Write-Host "Done setting output of $line"
        }
	}
